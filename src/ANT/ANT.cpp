...
void ANT::enablePwrCapabilities1(const uint8_t chan, const uint8_t capabilities)
{
    qDebug() << "Request to enable some capabilities #1 of power sensor 0x" << QString("%1").arg(capabilities, 2, 16, QChar('0')).toUpper();
    sendMessage(ANTMessage::enablePwrCapabilities1(chan, capabilities));
...
void ANT::enablePwrCapabilities2(const uint8_t chan, const uint8_t capabilities)
{
    qDebug() << "Request to enable some capabilities #2 of power sensor 0x" << QString("%1").arg(capabilities, 2, 16, QChar('0')).toUpper();
    sendMessage(ANTMessage::enablePwrCapabilities2(chan, capabilities));
...
ANT::channelInfo(int channel, int device_number, int device_id)
{
    ...
    if (!configuring && antChannel[channel]->is_power) {
        antChannel[channel]->capabilities();
        qDebug()<<"ANT power device found."<<device_number<<"on channel"<<channel;
    }
