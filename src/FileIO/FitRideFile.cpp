            ...
            case 44: /* rider_position_change */
                {
                    ...
                    switch (event_type) {
                        case 3:
                            ...
                            // // FIXME: determine whether it is encoded 16 or 32 bits
                            // p->number[0] = (data32 & 255);
                            // p->number[0] = (data16 & 255);

                            // positionXdata->datapoints.append(p);

                            // TODO:
                            // if previous was transition and current confirmed final position
                            //    -> replace previous element by final position and do not record this one
                            // if previous was transition and current do not confirm final position
                            //    -> delete previous element which is a wrong detection


                            qDebug() << QString("Rider position event received %1 type %2 data16 %3 data32 %4").arg(event).arg(event_type).arg(data16).arg(data32);

                            break;
                        default:
                            errors << QString("Unknown rider position change event %1 type %2 data %3 %4").arg(event).arg(event_type).arg(data16).arg(data32);
                            break;
                    }
                }
                break;
